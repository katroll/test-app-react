                    ...
                    return (
                        <ul key={user.first_name} className="relative w-full accordion-collapse collapse" id={`collapse-class-students-${spctc_class.id}`} aria-labelledby="sidenavSecEx2" data-bs-parent="#sidenavSecExample">
                            <li className="relative">
                                <div className={`flex justify-between text-sm py-2 px-5 bg-th-card-bg transition duration-300 ease-in-out ${index < spctc_class.users.length - 1 ? "border-b border-th-border" : null}`}>
                ...
                {spctc_class.quizzes.map((test, index) => {
                    return (
                        <ul key={test.name} className="relative w-full accordion-collapse collapse" id={`collapse-class-tests-${spctc_class.id}`} aria-labelledby="sidenavSecEx2" data-bs-parent="#sidenavSecExample">
                            <li className="relative">
                                <div className={`flex justify-between text-sm  py-2 px-5 bg-th-card-bg transition duration-300 ease-in-out ${index < spctc_class.quizzes.length - 1 ? "border-b border-th-border" : null}`}>
                                    <p>{test.name}</p>
                                    <svg 
                                        xmlns="http://www.w3.org/2000/svg" x="0px" y="0px"
                                        width="20" height="20"
