...
function EditClass({ classes }) {
    ...
    function handleClassEditChanges(e) {
        ...
        if(key === "studentsToAdd") {
            const students = [...classToEdit.studentsToAdd, e.target.value];
            setClassToEdit({...classToEdit, studentsToAdd: students});
            return;
        }
        if(key === "testsToAdd") {
            const tests = [...classToEdit.testsToAdd, e.target.value];
            setClassToEdit({...classToEdit, testsToAdd: tests});
            return;
        }
        setClassToEdit({...classToEdit, [key]: e.target.value})
    }
        ...
        <div className="flex flex-col w-full items-start">
            ...
            {editClass ? (
                <div className="flex w-full justify-start mb-3 pl-10 py-3 w-full bg-th-card-bg divide-x divide-th-border">
                    <form 
                        className="flex flex-col items-start w-1/2 space-y-2 justify-center"
                        onSubmit={null}
                    >
                        <div className="flex justify-center w-full space-x-2">
                            <label>Select Class to Edit: </label>
                            <select className="p-1 w-1/3 text-sm outline-none border border-th-border rounded" name="name" onChange={handleClassEditChanges}>
                                <option>Select</option>
                                {classes.map(spctc_class => {
                                    return (
                                        <option key={`${spctc_class.name}-${spctc_class.id}`} value={spctc_class.name} id="name">{spctc_class.name}</option>
                                    )
                                })}
                            </select> 
                        </div>
                        <div className="flex justify-center w-full space-x-2">
                            ...
                            <select className="p-1 w-1/3 text-sm outline-none border border-th-border rounded" name="studentsToAdd" onChange={handleClassEditChanges}>
                                ...
                                {students.map(student => {
                                    return (
                                        <option key={`${student.first_name}-${student.id}`} value={`${student.first_name} ${student.last_name}`}>{student.first_name} {student.last_name}</option>
                                    )
                                })}
                            </select> 
                        </div>
                        <div className="flex justify-center w-full space-x-2">
                            ...
                            <select className="p-1 w-1/3 text-sm outline-none border border-th-border rounded" name="testsToAdd" onChange={handleClassEditChanges}>
                                ...
                                {tests.map(test => {
                                    return (
                                        <option key={`${test.name}-${test.id}`} value={test.name}>{test.name}</option>
                                    )
                                })}
                            </select> 
                        </div>
